...
/* ---------------------------------------------------------------------------
   THE SIX METRICS — grounded in real, published indicators.

   Each one names the actual index it tracks, the United States' real current
   value and world rank, and a documented target. The standard is the same for
   all six: close the gap between where America is today and the benchmark.
   Where the US already leads, the target is the index's own theoretical best.

   Figures verified August 2026. Sources are shown on every metric card.
   --------------------------------------------------------------------------- */
const MAX_POINTS = 15000;        // 500 days × 30 = a flawless run
...
const METRICS = [
  {
    ...
    now: 32.38, target: 50, dp: 2, pre: "$", suf: "T", better: "high",
    rank: 1, of: 192, leader: "United States",
    targetNote: "A stretch target, not a published benchmark: $50T would be roughly 40% of world output.",
    context: "The United States is the world's largest economy by nominal GDP at $32.38 trillion — about a quarter of the $125 trillion world total, and larger than the next two economies combined. On a purchasing-power basis China is ahead."
  },
  {
    ...
    now: 0.938, target: 0.972, dp: 3, pre: "", suf: "", better: "high",
    rank: 18, of: 193, leader: "Iceland (0.972)",
    targetNote: "Target is Iceland's score — the highest in the world.",
    context: "The HDI combines life expectancy, schooling and income. America scores 0.938 and ranks 18th. Adjusted for inequality it falls to 29th, the steepest drop of any country in the top 20 — the gap is the point of the target."
  },
  {
    ...
    now: 0.0741, target: 0, dp: 4, pre: "", suf: "", better: "low",
    rank: 1, of: 145, leader: "United States",
    targetNote: "Global Firepower defines 0.0000 as a perfect score. Lower is stronger.",
  ...
  },
  {
    ...
    now: 74.9, target: 100, dp: 1, pre: "", suf: "/100", better: "high",
    rank: 1, of: 193, leader: "United States",
    targetNote: "Target is the index maximum of 100.",
  ...
  },
  {
    ...
    now: 61.7, target: 66, dp: 1, pre: "", suf: "/100", better: "high",
    rank: 3, of: 139, leader: "Switzerland (66.0)",
    targetNote: "Target is Switzerland's score — first in the world.",
  ...
  },
  {
    ...
    now: 489, target: 560, dp: 0, pre: "", suf: "", better: "high",
    rank: 16, of: 81, leader: "Singapore (~560)",
    targetNote: "Target is Singapore's mean across the three subjects — the world's best.",
...
// Progress is the share of the gap to the benchmark that has been closed.
function gapClosed(pts) { return clamp(pts / MAX_POINTS, 0, 1); }

// The projected value of a real indicator at the current level of discipline.
function metricValue(m, pts) { return m.now + gapClosed(pts) * (m.target - m.now); }

function fmtMetric(m, v) {
  const n = v.toFixed(m.dp);
  const withCommas = m.dp === 0 ? Number(n).toLocaleString() : n;
  return m.pre + withCommas + m.suf;
}

// Rank improves as the gap closes; a metric already at #1 simply holds it.
function metricRank(m, pts) {
  if (m.rank === 1) return 1;
  const g = gapClosed(pts);
  return Math.max(1, Math.round(m.rank - (m.rank - 1) * g));
}
...
function metricTierName(pct) {
  if (pct >= 100) return "Benchmark reached";
  if (pct >= 75) return "Closing in";
  if (pct >= 50) return "Over halfway";
  if (pct >= 25) return "Gaining";
  if (pct >= 5) return "Moving";
  return "Holding";
}
...
function renderMetrics(pts) {
  ...
  $("#indexFill").style.width = (runComplete() ? 100 : pct) + "%";
  $("#indexNum").textContent = pct.toFixed(1) + "% of the gap closed";
  $("#indexCap").textContent = rank === 1
    ? "National Power Index · every benchmark met"
    : "National Power Index · distance to the world's best, across six real indicators";

  $("#metrics").innerHTML = METRICS.map(m => {
    const v = metricValue(m, pts);
    const r = metricRank(m, pts);
    return '<button class="metric" data-metric="' + m.key + '" aria-label="' + esc(m.name) + ' — tap for detail">' +
      ...
      '<span class="metric-sub">' + m.indicator + "</span>" +
      '<span class="metric-track"><i style="width:' + pct + '%"></i></span>' +
      '<span class="metric-foot"><span class="metric-rank">#' + r + " of " + m.of + "</span>" +
      '<span class="metric-tier">' + metricTierName(pct) + "</span></span></button>";
  }).join("");
}
...
function openMetric(key) {
  ...
  const pts = totalPoints();
  const v = metricValue(m, pts);
  const pct = gapClosed(pts) * 100;
  const dir = m.better === "low" ? "Lower is better." : "Higher is better.";
  ...
  $("#sheetScroll").innerHTML =
    '<div class="metric-sheet">' +
      ...
      '<div class="ms-scale">' +
        '<div class="ms-row"><span>United States today</span><b>' + fmtMetric(m, m.now) + "</b></div>" +
        '<div class="ms-row is-you"><span>Your projection</span><b>' + fmtMetric(m, v) + "</b></div>" +
        '<div class="ms-row"><span>Benchmark</span><b>' + fmtMetric(m, m.target) + "</b></div>" +
      "</div>" +
      '<div class="ms-bar"><i style="width:' + pct + '%"></i></div>' +
      '<p class="ms-pct">' + pct.toFixed(1) + "% of the gap closed &middot; " + dir + "</p>" +

      '<div class="dos-meta">' +
        "<div><b>#" + m.rank + "</b><span>Real rank today</span></div>" +
        "<div><b>#" + metricRank(m, pts) + "</b><span>Your projection</span></div>" +
        "<div><b>" + m.of + "</b><span>Countries ranked</span></div>" +
      "</div>" +

      "<p>" + esc(m.context) + "</p>" +
      '<p class="ms-target"><b>The standard.</b> ' + esc(m.targetNote) + "</p>" +
      '<p class="ms-note">Leader today: ' + esc(m.leader) + ". A flawless 500-day run is " +
      MAX_POINTS.toLocaleString() + " points, which closes the gap completely. " +
      "Your points move this projection and nothing else — the real-world figures above are fixed, published data.</p>" +
    "</div>";
...
/* ==========================================================================
   ...
   page underneath.
   ========================================================================== */
let lbSet = [], lbAt = 0, lbZoomed = false;

function openLightbox(list, index) {
  lbSet = list; lbAt = index || 0; lbZoomed = false;
...
function paintLightbox() {
  ...
  $("#lbPrev").hidden = !many;
  $("#lbNext").hidden = !many;
  $("#lbHint").textContent = many
    ? "Tap the image to zoom · swipe or use arrow keys"
...
function closeLightbox() {
  ...
  setZoom(false);
  // the dossier sheet may still be open underneath
  document.body.style.overflow = $("#sheetScrim").hidden ? "" : "hidden";
}

// Collect every photo in the container that was clicked, so the arrows walk
// the whole set rather than just the one that was tapped.
...
function wireLightbox() {
  ...
  $("#lbZoom").addEventListener("click", () => setZoom(!lbZoomed));
  $("#lbImg").addEventListener("click", () => setZoom(!lbZoomed));
  ...
  $("#lbStage").addEventListener("touchend", e => {
    if (lbZoomed) return;                       // panning, not paging
    const dx = e.changedTouches[0].clientX - tx;
    const dy = e.changedTouches[0].clientY - ty;
    if (Math.abs(dx) > 55 && Math.abs(dx) > Math.abs(dy) * 1.4) moveLightbox(dx < 0 ? 1 : -1);
  ...
  document.addEventListener("keydown", e => {
    ...
    else if (e.key === " " || e.key === "Enter") { e.preventDefault(); setZoom(!lbZoomed); }
  }, true);
}
